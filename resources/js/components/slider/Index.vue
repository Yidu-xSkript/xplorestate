  ...
  >
    ...
    >
      ...
      >
        ...
        >
          <slot name="tooltip2">
            <span
              :style="tooltip2Styles"
              class="vue-slide-bar-tooltip-bottom-arrow"
            >
              {{ val }}
              <small v-if="tooltipSuffix !== null">{{ tooltipSuffix }}</small>
            </span>
          </slot>
        </span>
      </div>
    ...
    <div v-if="range" class="vue-slide-bar-range">
      <div
        ...
        :key="index"
        :class="{ 'vue-slide-bar-separate': !r.isHide }"
        :style="dataLabelStyles"
      >
        <span v-if="!r.isHide" class="vue-slide-bar-separate-text">
          {{ r.label }}<br /><span v-if="tooltipNote !== null">{{
            tooltipNote
          }}</span>
        </span>
      </div>
...
export default {
  ...
  methods: {
    ...
    setTransitionTime(time) {
      ...
      this.slider2.style.transitionDuration = `${time}s`;
      this.slider2.style.WebkitTransitionDuration = `${time}s`;
...
.vue-slide-bar-tooltip-bottom {
  bottom: -35px;
  left: 40%;
  transform: translate(-50%, -100%);
}
.vue-slide-bar-tooltip,
.vue-slide-bar-tooltip-bottom-arrow {
  position: relative;
  font-size: 14px;
  white-space: nowrap;
  padding: 2px 5px;
...
.vue-slide-bar-tooltip-bottom-arrow::before {
  content: "";
  position: absolute;
  bottom: -10px;
  left: 50%;
